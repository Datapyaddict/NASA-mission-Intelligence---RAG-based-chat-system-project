...
class ChromaDBRAGSystem:
    ...
    def __init__(self, embedding_config: str = "openai_embeddings", persist_directory: str = "./chroma_db"):
        ...
        if embedding_config not in EMBEDDING_CONFIGS:
        ...
        if self.embedding_config["provider"] == "sentence_transformers":
            try:
                from sentence_transformers import SentenceTransformer
            except ImportError as e:
                raise ImportError(
                    "sentence-transformers is required for local_embeddings. "
                    "Install it with: pip install sentence-transformers"
                ) from e

            self.st_model = SentenceTransformer(self.embedding_config["model"])
    ...
    def create_collection(self, collection_key: str) -> chromadb.Collection:
        ...
        try:
            ...
            # Always disable Chroma-side embedding because this script supplies embeddings
            # explicitly via `collection.add(embeddings=...)` and `collection.query(query_embeddings=...)`.
        ...
        try:
            if self.embedding_config["provider"] == "openai":
                ...
                if self.st_model is None:
                    raise RuntimeError(
                        "Local embedding model is not initialized. "
                        "Ensure embedding_config='local_embeddings' and sentence-transformers is installed."
                    )

                embeddings = self.st_model.encode(texts, convert_to_numpy=True).tolist()
